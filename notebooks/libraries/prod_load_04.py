...
dbutils.widgets.text("DeltaIntermediatePath","", "DeltaIntermediatePath")
DeltaIntermediatePath = dbutils.widgets.get("DeltaIntermediatePath")
...
dbutils.widgets.text("RS_load_flag","", "RS_load_flag")
RS_load_flag = dbutils.widgets.get("RS_load_flag")

dbutils.widgets.text("folder_name","", "folder_name")
folder_name = dbutils.widgets.get("folder_name")

dbutils.widgets.text("unity_catalog_info","", "unity_catalog_info")
unity_catalog_info = dbutils.widgets.get("unity_catalog_info")
...
DeltaIntermediatePath = dict(x.split('=',1) for x in DeltaIntermediatePath.split(','))
...
try:
  ...
  interim_dfs = {}
  interim_count_dfs = {}
  for k,v in DeltaIntermediatePath.items():
    print(k,v)
    interim_dfs[str(k)] = spark.read.format('delta').load(v)
    interim_count_dfs[str(k)] = interim_dfs[str(k)].count()
###########################################################################################################
except Exception as e:
  print(e)
  exit_notebook(STEP,e)
...
write_as_delta(df = interim_dfs[str(k)],path = v,mode = 'overwrite',compression_type = 'snappy',overwriteSchema=True,optimize = True,vacuum = True,\
table_name = f"delta.`{v}`")
final_dfs[str(k)] = spark.read.format('delta').load(v)
count_dfs[str(k)] = final_dfs[str(k)].count()

# COMMAND ----------

#default load to s3_final bucket
try:
  STEP = 'Copy intermediate data to final location'
  for k,v in DeltaFinalPath.items():
    print(f"Copying to final location from intermediate loacation for {k if k else folder_name}")
    write_as_delta(df = interim_dfs[str(k)],path = v,mode = 'overwrite',compression_type = 'snappy',overwriteSchema=True,optimize = True,vacuum = True,\
      table_name = f"delta.`{v}`")
    final_dfs[str(k)] = spark.read.format('delta').load(v)
    count_dfs[str(k)] = final_dfs[str(k)].count()
...
final_load = "FAILED"
#count check after loading to final location
for k,v in DeltaFinalPath.items():
  if count_dfs[str(k)] != interim_count_dfs[str(k)]:
    final_load = "FAILED"
  else:
    final_load = "PASSED"
...
#deleting the intermediate delta if final copy is success
if final_load == "PASSED":
  intermediate_cleanup = False
  try:
    STEP = "Deleting intermediate delta files"
    for k,v in DeltaIntermediatePath.items():
      parts = v.split('/')
      if folder_name and "intermediate" in parts:
        dbutils.fs.rm(v,recurse=True)
        print(f"Deleted intermediate files for {k if k else folder_name}")
...
if RS_load_flag.lower() == "true" and final_load == "PASSED" and intermediate_cleanup:
  try:
  ...
  print("Data not loaded to Redshift")

# COMMAND ----------

#copying the data to external location 
# if final_load == "PASSED":
#   for k,v in DeltaFinalPath.items():
#     if len(RedshiftFinalTable[k]) > 1:
#       table_name = RedshiftFinalTable[k].split(".")[1]
#       sql_query = f"CREATE TABLE IF NOT EXISTS {unity_catalog_info}.{table_name} USING DELTA LOCATION '{v}'"
#       spark.sql(sql_query)
#     else:
#       table_name = RedshiftFinalTable[folder_name].split(".")[1]
#       sql_query = f"CREATE TABLE IF NOT EXISTS {unity_catalog_info}.{table_name} USING DELTA LOCATION '{v}'"
#       spark.sql(sql_query)
...
###########################################################################################################
if RS_load_flag.lower() == "true" and final_load == "PASSED":
  try:
