...
try:
  ...
  vital_keys = ['enrich_app_runtime', 'enrich_bat_runtime', 'enrich_dq_summary_runtime', 'enrich_dq_test_runtime', 'enrich_dw_load_runtime', 'enrich_dw_test_runtime', 'enrich_end_time', 'enrich_environment', 'enrich_final_output', 'enrich_good_percentage', 'enrich_job_name', 'enrich_job_status', 'enrich_lot_id', 'enrich_pre_check_rs_runtime', 'enrich_processing_date', 'enrich_source_type', 'enrich_start_time', 'enrich_step_error', 'enrich_total_error_count', 'enrich_total_error_count_percent', 'enrich_total_final_count', 'enrich_total_final_size', 'enrich_total_flags_size', 'enrich_total_good_count', 'enrich_total_inc_count', 'enrich_total_inc_hist_count', 'enrich_total_inc_size', 'enrich_total_mod_count', 'enrich_total_mod_count_percent', 'enrich_velocity_in_records_per_second', 'enrich_workflow_runtime', 'enrich_workflow_step']
  ...
  for k, v in final_dict.items():
    ...
    if k in keys1:
      ...
      if k in ['enrich_app_runtime','enrich_workflow_runtime','enrich_dw_load_runtime','enrich_total_flags_size','enrich_total_inc_hist_count'] :
        tmp_msg = '******' + k + (50-len(k))*' ' + str(v)
        validationMsg.append(tmp_msg)
      else:
        tmp_msg = k + (50-len(k))*' ' + str(v)
        validationMsg.append(tmp_msg)
        
  tmp_msg = '****** test_run_time ' + (50-len(k))*' ' + str(test_run_time)     
  validationMsg.append(tmp_msg)
